...
exports.registerUser = async (req, res) => {
  ...
  if (user) {
    ...
    return res.status(500).send({ status: false, message: errorMessage });
  }
...
`;
    ...
    return res
      .status(500)
      .send({ status: false, message: "User registration failed" });
  }
};
...
exports.loginUser = async (req, res) => {
  ...
  try {
    ...
    if (user.status !== 1) {
      return res.status(404).send({
        ...
        message: `Your Account is ${
          user.status === 2 ? "blocked" : user.status === 0 ? "Deactivated" : ""
        }.`,
      });
    }
    ...
    console.log(error);
    return res.status(500).send({ status: false, message: "Login failed" });
  }
};
