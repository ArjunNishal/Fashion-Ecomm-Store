...
const EditProduct = () => {
  ...
  useEffect(() => {
    ...
    if (
      ...
      !decoded.permissions.includes("Edit Products")
    ) {
  ...
  const fetchProduct = async () => {
    try {
      ...
      setproductForm({
        ...
        categories: productdetails.categories.map((el) => ({
          value: el._id,
          label: el.name,
        })),
        price: JSON.stringify(productdetails.price),
        actualPrice: JSON.stringify(productdetails.actualPrice),
        discount: JSON.stringify(productdetails.discount),
        products: productdetails.products,
        thumbnail: null,
        previousThumbnail: productdetails.thumbnail,
        images: [],
        previousImages: productdetails.images,
        note: productdetails.note,
        featureProduct: productdetails.featureProduct,
        // shortDescription: productdetails.shortDescription,
        // description: productdetails.description,
      });
      setDescriptionContent(productdetails.description);
      setShortDescriptionContent(productdetails.shortDescription);
      setloading(false);
    } catch (error) {
      setloading(false);
      console.error("Error fetching categories:", error);
  ...
  useEffect(() => {
    console.log(productForm, ShortDescImages, DescImages, "productform");
  ...
  const removePreviousImage = (index) => {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        const images = [...productForm.previousImages];
        images.splice(index, 1);
        setproductForm((prevState) => ({
          ...prevState,
  ...
  const validateForm = () => {
    ...
    // Validate thumbnail
    if (!productForm.thumbnail && !productForm.previousThumbnail)
      errors.thumbnail = "Thumbnail is required";

    // Validate product images
    if (
      productForm.images.length === 0 &&
      !productForm.previousImages.length === 0
    )
    ...
    if (!productForm.shortDescription && !ShortDescriptionContent)
      errors.shortDescription = "Short Description is required";

    // Validate description
    if (!productForm.description && !ShortDescriptionContent)
  ...
  const handleSubmit = async (e) => {
    ...
    if (validateForm()) {
      ...
      formData.append(`oldimages`, productForm.previousImages);
      formData.append("featureProduct", productForm.featureProduct);

      try {
        setloading(true);
        const res = await axiosInstance.put(
          `panel/product/editproduct/${pid}`,
          formData,
          {
            headers: {
              Authorization: `Bearer ${token}`,
            ...
            <div className="create_product_form_main">
              <div className="card border-0 shadow">
                <div className="card-header">
                  <h6 className="m-0">Edit New Product</h6>
                </div>
                        ...
                        <div className="col-12">
                          <div className="product_images_main_box">
                            <div className="row mx-0">
                              ...
                              <div className="col-md-6 d-flex flex-wrap col-12">
                                    ...
                                    {productForm?.thumbnail && (
                                      <>
                                        ...
                                        <img
                                          ...
                                          // src={`${renderUrl}uploads/product/${productForm.previousThumbnail}`}
                                          alt="thumbnail"
                                        />
                                      </>
                                    )}
                              ...
                              <div className="col-12">
                                ...
                                <div className="product_images_row">
                                  <div className="d-flex flex-wrap">
                                    {productForm?.previousImages?.map(
                                      (image, index) => (
                                        <div
                                          key={index}
                                          className="product_image_panel"
