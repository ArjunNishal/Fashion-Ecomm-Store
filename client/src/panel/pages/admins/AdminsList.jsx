...
const AdminsList = () => {
  ...
  const fetchAdmins = async () => {
    try {
      setloading(true);
      const response = await axiosInstance.get(`panel/user/get/admins`, {
        params: {
          page: currentPage,
          limit: itemsPerPage,
        },
  ...
  const toggleSurveyStatus = async (id, activate) => {
    try {
      const response = await axiosInstance.patch(
        `panel/user/edit/admin/${id}/status`,
        {
          status: activate,
        },
        {
            ...
            <div className="admins_table">
              <div className="card table_card border-0 shadow">
                <div className="card-header row mx-0 align-items-center border-0">
                  <h6 className="col-md-4 col-12">Admins List</h6>
                ...
                {admins.length === 0 && (
                  <>
                    <div className="text-center">
                      <small className="text-small">No Admins Found</small>
                    </div>
                  </>
                )}
                {selectedAdmin && (
